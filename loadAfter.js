...
function renderExercises(exercises) {
    ...
    container.innerHTML = generateContentHTML(exercises);
}

function handleSearch() {
    const searchQuery = document.getElementById('search').value.toLowerCase();
    const filteredExercises = allExercises.filter(exercise => {
        return exercise.exercise.sentence.toLowerCase().includes(searchQuery) ||
            exercise.exercise.translation.toLowerCase().includes(searchQuery) ||
            (exercise.exercise.notes && exercise.exercise.notes.some(note => note.toLowerCase().includes(searchQuery)));
    });
