...
function initializeEventListeners() {
    ...
    const useLessonsCheckbox = document.getElementById('useLessons');
    useLessonsCheckbox.checked = useLessons;
    useLessonsCheckbox.dispatchEvent(new Event('change')); // Trigger change event
}
...
// Create a mapping of Kanji characters to their rank based on the selected list
const kanjiRanking = kanjiOrderList.reduce((ranking, kanji, index) => {
    ranking[kanji] = index;
    return ranking;
}, {});
...
function renderResults(results) {
    ...
    // Sort the results based on the predefined ranking
    results.sort((a, b) => {
        const rankA = kanjiRanking[a.literal] !== undefined ? kanjiRanking[a.literal] : Infinity;
        const rankB = kanjiRanking[b.literal] !== undefined ? kanjiRanking[b.literal] : Infinity;
        return rankA - rankB;
    });
